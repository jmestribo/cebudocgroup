...
class Panel extends CI_Controller {
	...
	function updateremarks(){
	  ...
	  function getfile(){
	    ...
	    $name = $file.'.pdf';
	    $data = file_get_contents("https://cebudocgroup.com/upload/cif/".$name.""); // Read the file's contents
	...
	function submitremarks(){
		...
		$this->create->remarksnew($data);
		redirect("Panel/Lists");
	}
	...
	function allregisteredonline($type){

		$type = $this->input->post('type');
		
		$requestlist = $this->get->getallappointment(0);
		...
		if (count($requestlist) > 0) {
			...
			foreach ($requestlist as $time) {
                ...
                if ($time->attachfile != "") {
                	$file = '<center><a href="https://cebudocgroup.com/onlineregistration/upload/attachment/'.$time->attachfile.'.jpg" target="_blank" class="badge badge-primary badge-block text-uppercase" >Check attachfile</a></center>';
                }else{
                	$file = "";
                }
                if ($_SESSION['department'] == "cashier") {
				...
				if ($time->firstname != "") {
					...
					if (strtotime($today1) > strtotime($time->date)) {
						...
						$list .='
							<tr class="editpatient '.$css.' context-menu-one " patientsched="'.$time->schedid.'" patientid="'.$time->pid.'">
								<td style="width: 1vw !important;" >'.$count.'</td>
								<td style="width: 5vw !important;" >'.date('F d Y' ,strtotime($time->date)).'</td>
								<td style="width: 3vw !important;" >'.$time->time.'</td>
